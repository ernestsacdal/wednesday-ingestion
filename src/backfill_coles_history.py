"""One-time (re-runnable) backfill of Coles half-price HISTORY from hotprices.

The recurring refresh (src/refresh_coles_hotprices.py) writes only *this week's*
currently-half Coles specials. This script imports the deep history: for every
Coles product that has ever gone half-price, it ensures the product row exists
and writes one specials + one price_observation row per historical half-price
event (source='hotprices'). That gives the cycle predictor real multi-year
history per product (instead of the ~6 weeks we'd otherwise have), so genuine
medium/high-confidence predictions can emerge immediately.

Idempotent: ON CONFLICT DO NOTHING on (product_id, week_start) for specials and
(product_id, observed_at, source) for price_observations. Safe to re-run; the
weekly refresh keeps current data fresh thereafter. The CURRENT week is skipped
here (the refresh owns it) to avoid double-writing the live promo.

Run locally (one-off):
    python -m src.backfill_coles_history --verbose
Requires SUPABASE_DB_URL.
"""
from __future__ import annotations

import argparse
import logging
import os
import sys
from datetime import datetime, timedelta, timezone
from pathlib import Path

import psycopg

from src.db.writer import _synthetic_sku
from src.scrapers.base import configure_logging
from src.scrapers.hotprices import (
    _most_recent_wednesday, fetch_dump, parse_products,
)

_PRODUCT_BATCH = 500
_ROW_BATCH = 1000


def _chunks(seq, n):
    for i in range(0, len(seq), n):
        yield seq[i:i + n]


def _upsert_products(cur, products, log) -> None:
    """Ensure a product row exists for every ever-half Coles product.

    Dedup by synthetic SKU first: distinct products whose names normalise to the
    same key would otherwise hit the same ON CONFLICT target twice in one
    multi-row statement (Postgres rejects that). Their events still merge under
    the shared product_id in the caller (ON CONFLICT on (product_id, week_start)).
    """
    by_sku: dict[str, tuple] = {}
    for p in products:
        sku = _synthetic_sku("coles", p.name)
        by_sku[sku] = (
            "coles", sku, p.name, "Uncategorised",
            p.regular_cents, p.image_url, p.source_product_url,
        )
    rows = list(by_sku.values())
    total = 0
    for batch in _chunks(rows, _PRODUCT_BATCH):
        ph = ",".join(["(%s,%s,%s,%s,%s,%s::text,%s,now(),now())"] * len(batch))
        flat = [v for r in batch for v in r]
        cur.execute(
            f"""
            insert into products
                (retailer, retailer_sku, name, category, regular_price_cents,
                 image_url, source_product_url, image_fetched_at, last_seen)
            values {ph}
            on conflict (retailer, retailer_sku) do update set
                image_url = coalesce(products.image_url, excluded.image_url),
                source_product_url = coalesce(products.source_product_url, excluded.source_product_url),
                image_fetched_at = case
                    when products.image_url is null and excluded.image_url is not null then now()
                    else products.image_fetched_at end,
                last_seen = now()
            """,
            flat,
        )
        total += len(batch)
    log.info("backfill_history.products_upserted=%d", total)


def _load_product_ids(cur) -> dict[str, str]:
    cur.execute("select retailer_sku, id::text from products where retailer='coles'")
    return {sku: pid for sku, pid in cur.fetchall()}


def backfill(*, db_url: str, log: logging.Logger) -> dict[str, int]:
    raw = fetch_dump("coles", log=log)
    today = datetime.now(timezone.utc).date()
    products = parse_products(raw, log=log, today=today)
    ever_half = [p for p in products if p.events]
    current_week = _most_recent_wednesday(today)
    log.info("backfill_history.start ever_half=%d current_week=%s", len(ever_half), current_week)

    specials_rows: list[tuple] = []
    obs_rows: list[tuple] = []

    with psycopg.connect(db_url, connect_timeout=30) as conn:
        with conn.cursor() as cur:
            _upsert_products(cur, ever_half, log)
        conn.commit()
        with conn.cursor() as cur:
            ids = _load_product_ids(cur)
        log.info("backfill_history.product_ids_loaded=%d", len(ids))

        skipped_no_id = 0
        for p in ever_half:
            pid = ids.get(_synthetic_sku("coles", p.name))
            if pid is None:
                skipped_no_id += 1
                continue
            for ev in p.events:
                # The refresh owns the live promo; skip current-week events here.
                if ev.on_date >= current_week:
                    continue
                week_end = ev.on_date + timedelta(days=6)
                specials_rows.append((
                    pid, ev.on_date, week_end, ev.regular_cents, ev.sale_cents,
                    ev.discount_pct, True, "hotprices",
                ))
                obs_rows.append((
                    pid, ev.sale_cents, True, ev.discount_pct, ev.on_date, "hotprices",
                ))

        spec_written = 0
        with conn.cursor() as cur:
            for batch in _chunks(specials_rows, _ROW_BATCH):
                ph = ",".join(["(%s,%s,%s,%s,%s,%s,%s,%s)"] * len(batch))
                flat = [v for r in batch for v in r]
                cur.execute(
                    f"""
                    insert into specials
                        (product_id, week_start, week_end, regular_price_cents,
                         sale_price_cents, discount_pct, is_half_price, source)
                    values {ph}
                    on conflict (product_id, week_start) do nothing
                    """,
                    flat,
                )
                spec_written += cur.rowcount
        conn.commit()

        obs_written = 0
        with conn.cursor() as cur:
            for batch in _chunks(obs_rows, _ROW_BATCH):
                ph = ",".join(["(%s,%s,%s,%s,%s,%s)"] * len(batch))
                flat = [v for r in batch for v in r]
                cur.execute(
                    f"""
                    insert into price_observations
                        (product_id, price_cents, is_special, discount_pct, observed_at, source)
                    values {ph}
                    on conflict (product_id, observed_at, source) do nothing
                    """,
                    flat,
                )
                obs_written += cur.rowcount
        conn.commit()

    # NOTE: purging the old (inaccurate) StockUp Coles rows is a separate,
    # explicit step (src/purge_stockup_coles.py) — this backfill is additive only.

    stats = {
        "ever_half": len(ever_half),
        "event_rows": len(specials_rows),
        "specials_inserted": spec_written,
        "observations_inserted": obs_written,
        "skipped_no_id": skipped_no_id,
    }
    log.info("backfill_history.done %s", stats)
    return stats


def _load_dotenv() -> None:
    for env_path in (Path.cwd() / ".env", Path(__file__).resolve().parent.parent / ".env"):
        if not env_path.is_file():
            continue
        for raw in env_path.read_text(encoding="utf-8").splitlines():
            line = raw.strip()
            if not line or line.startswith("#") or "=" not in line:
                continue
            k, _, v = line.partition("=")
            k = k.strip(); v = v.strip().strip('"').strip("'")
            if k and k not in os.environ:
                os.environ[k] = v
        break


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="backfill_coles_history")
    parser.add_argument("--verbose", "-v", action="store_true")
    args = parser.parse_args(argv)
    log = configure_logging(verbose=args.verbose)

    if not os.environ.get("SUPABASE_DB_URL"):
        _load_dotenv()
    db_url = os.environ.get("SUPABASE_DB_URL")
    if not db_url:
        log.error("SUPABASE_DB_URL not set (env or .env file)")
        return 2

    stats = backfill(db_url=db_url, log=log)
    return 0 if stats["specials_inserted"] >= 0 else 1


if __name__ == "__main__":
    sys.exit(main())
